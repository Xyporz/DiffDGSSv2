...
class FreqFusion(nn.Module):
    def __init__(self,
        ...
        self.hr_channel_compressor = nn.Sequential(
            nn.Conv2d(hr_channels, self.compressed_channels, 1),
            ccbn(self.compressed_channels, len(self.time_steps), nn.Linear, eps=1e-4, norm_style='bn')
        )
        
        self.lr_channel_compressor = nn.Sequential(
            nn.Conv2d(lr_channels, self.compressed_channels, 1),
            ccbn(self.compressed_channels, len(self.time_steps), nn.Linear, eps=1e-4, norm_style='bn')
        )
        ...
        self.content_encoder = nn.Sequential(
            nn.Conv2d( # ALPF generator
                ...
                groups=1),
            ccbn(lowpass_kernel ** 2 * self.up_group * self.scale_factor * self.scale_factor, 
                 len(self.time_steps), nn.Linear, eps=1e-4, norm_style='bn')
        )
        ...
        if self.use_high_pass:
            self.content_encoder2 = nn.Sequential(
                nn.Conv2d( # AHPF generator
                    ...
                    dilation=self.encoder_dilation,
                    groups=1),
                ccbn(highpass_kernel ** 2 * self.up_group * self.scale_factor * self.scale_factor, 
                     len(self.time_steps), nn.Linear, eps=1e-4, norm_style='bn')
            )
    ...
    def init_weights(self):
        ...
        normal_init(self.content_encoder[0], std=0.001)
        if self.use_high_pass:
            normal_init(self.content_encoder2[0], std=0.001)
    ...
    def forward(self, hr_feat, lr_feat, t=None, use_checkpoint=False):
        if use_checkpoint:
            return checkpoint(self._forward, hr_feat, lr_feat, t)
        else:
            return self._forward(hr_feat, lr_feat, t)

    def _forward(self, hr_feat, lr_feat, t=None):
        if t is None:
            t = torch.tensor([0]).to(hr_feat.device)
        ...
        if t_value in self.time_steps:
            ...
            t_idx = 0
            print(f"警告: 时间步 {t_value} 不在预定义列表中，使用默认索引 0")
            
        t_cond = one_hot(torch.tensor([t_idx]).to(hr_feat.device), len(self.time_steps))
        
        compressed_hr_feat = hr_feat
        for layer in self.hr_channel_compressor:
            if isinstance(layer, ccbn):
                compressed_hr_feat = layer(compressed_hr_feat, t_cond)
            else:
                compressed_hr_feat = layer(compressed_hr_feat)
                
        compressed_lr_feat = lr_feat
        for layer in self.lr_channel_compressor:
            if isinstance(layer, ccbn):
                compressed_lr_feat = layer(compressed_lr_feat, t_cond)
            else:
                compressed_lr_feat = layer(compressed_lr_feat)
                
        if self.semi_conv:
            if self.comp_feat_upsample:
                if self.use_high_pass:
                    mask_hr_hr_feat = compressed_hr_feat
                    for layer in self.content_encoder2:
                        if isinstance(layer, ccbn):
                            mask_hr_hr_feat = layer(mask_hr_hr_feat, t_cond)
                        else:
                            mask_hr_hr_feat = layer(mask_hr_hr_feat)
                    ...
                    mask_lr_hr_feat = compressed_hr_feat
                    for layer in self.content_encoder:
                        if isinstance(layer, ccbn):
                            mask_lr_hr_feat = layer(mask_lr_hr_feat, t_cond)
                        else:
                            mask_lr_hr_feat = layer(mask_lr_hr_feat)
                            
                    mask_lr_init = self.kernel_normalizer(mask_lr_hr_feat, self.lowpass_kernel, hamming=self.hamming_lowpass)
                    
                    mask_lr_lr_feat_lr = compressed_lr_feat
                    for layer in self.content_encoder:
                        if isinstance(layer, ccbn):
                            mask_lr_lr_feat_lr = layer(mask_lr_lr_feat_lr, t_cond)
                        else:
                            mask_lr_lr_feat_lr = layer(mask_lr_lr_feat_lr)
                    ...
                    temp_feat = compressed_lr_feat
                    for layer in self.content_encoder2:
                        if isinstance(layer, ccbn):
                            temp_feat = layer(temp_feat, t_cond)
                        else:
                            temp_feat = layer(temp_feat)
                            
                    mask_hr_lr_feat = F.interpolate(
                        carafe(temp_feat, mask_lr_init, self.lowpass_kernel, self.up_group, 2), size=compressed_hr_feat.shape[-2:], mode='nearest')
                    mask_hr = mask_hr_hr_feat + mask_hr_lr_feat
                else: raise NotImplementedError
            else:
                temp_feat = compressed_hr_feat
                for layer in self.content_encoder:
                    if isinstance(layer, ccbn):
                        temp_feat = layer(temp_feat, t_cond)
                    else:
                ...
                mask_lr = temp_feat + F.interpolate(
                ...
                if self.use_high_pass:
                    ...
                    mask_hr = temp_feat + F.interpolate(
            ...
            mask_lr = compressed_x
            for layer in self.content_encoder:
                if isinstance(layer, ccbn):
                    mask_lr = layer(mask_lr, t_cond)
                else:
                    mask_lr = layer(mask_lr)
                    
            if self.use_high_pass: 
                mask_hr = compressed_x
                for layer in self.content_encoder2:
                    if isinstance(layer, ccbn):
                        mask_hr = layer(mask_hr, t_cond)
                    else:
                        mask_hr = layer(mask_hr)
